...
import { getCollectorProfiles } from 'apiSdk/collector-profiles';
...
function DashboardPage() {
  ...
  const collectorProfileStat = useSWR<PaginatedInterface<CollectorProfileInterface>>(
    () => `/collector-profiles`,
    () => getCollectorProfiles({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    },
    {
      ...
      result: collectorProfileStat?.data?.totalCount,
      isLoading: collectorProfileStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
